...
### Core Commands
- `hg_status`: Show working directory status (like `git status`)
- `hg_log`: Show commit history (like `git log`)
...
- `hg_bookmarks`: List bookmarks (lightweight branches, like Git branches in hg-git)
- `hg_topic`: Create a topic (lightweight branch)
- `hg_topics`: List all topics
- `hg_topic_current`: Show current topic
...
- `hg_resolve`: List merge conflicts
...
### Configuration & Help
- `hg_config`: Show Mercurial configuration (check for hg-git extension)
